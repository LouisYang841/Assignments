...
**Justification:** Chose **B**. Adding new behaviors does not require changes in existing classes, this adheres to the **Open-Closed Principle(OCP)**. In contrast, **Alternative A** will require change in `playTurn` of the creatures, this is against **OCP**. This also provided the extension point for RRQ4 to opt in by swapping behavior tree items to actively control behavior.
