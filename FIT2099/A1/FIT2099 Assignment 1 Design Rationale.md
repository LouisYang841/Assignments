...
**Justification**: Chose **B**. **Alternative A** directly interact with the responders, every new consequence requires modification in `AlarmSystem`, which is **against Open-Closed Principle**. **Alternative C** centeralize the consequence logic, but at the cost of breaking responding `GameEntity`'s encapsulation, which is **against the Single Responsibility Principle**. 
