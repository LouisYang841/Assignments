...
**Alternative B:** use a `TreeMap<Integer, Behaviour>` in every creature with a behaviour. Create behaviour classes such as `AttackNearbyBehavior` that implements `Behavior`. For undead, the `AttackNearbyBehavior` will be prioritzed over the `WanderBehavior`. For Slime, the `ConsumeBehaviour` will be prioritized over the `WanderBehaviour`. `playTurn()` will check the map and find the first `Behaviour` that is not yielding `null`.
